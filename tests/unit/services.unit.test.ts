...
import { 
  ...
  MockGlobalPoliciesRepository, 
  MockDefaultPreferencesRepository 
} from '../mocks/repositories';

describe('Domain Services Unit Tests (without database)', () => {
  let userPrefsRepo: MockUserPreferencesRepository;
  let globalPoliciesRepo: MockGlobalPoliciesRepository;
  let defaultPrefsRepo: MockDefaultPreferencesRepository;
  ...
  beforeEach(() => {
    userPrefsRepo = new MockUserPreferencesRepository();
    globalPoliciesRepo = new MockGlobalPoliciesRepository();
    defaultPrefsRepo = new MockDefaultPreferencesRepository();
    
    // Настраиваем дефолтные настройки
    defaultPrefsRepo.setDefaults([
    ...
    // Также устанавливаем дефолтные настройки в userPrefsRepo
    userPrefsRepo.setDefaultPreferences([
      { notificationType: NotificationType.TRANSACTIONAL_EMAIL, channel: Channel.EMAIL, enabled: true },
      { notificationType: NotificationType.MARKETING_EMAIL, channel: Channel.EMAIL, enabled: false },
      { notificationType: NotificationType.TRANSACTIONAL_SMS, channel: Channel.SMS, enabled: true },
      { notificationType: NotificationType.MARKETING_SMS, channel: Channel.SMS, enabled: false },
  ...
  describe('UserPreferencesService', () => {
    test('should create default preferences for new user', async () => {
      // Убедимся, что дефолтные настройки установлены
      const defaults = await defaultPrefsRepo.getAllDefaults();
      ...
      expect(preferences.preferences).toHaveLength(4);
  ...
  describe('NotificationEvaluationService', () => {
    ...
    test('should use default preferences when user has no preferences', async () => {
      // Пользователь без настроек
      const request = {
        userId: 'new-user-without-preferences',
        notificationType: NotificationType.MARKETING_EMAIL,
        channel: Channel.EMAIL,
        region: Region.US,
        datetime: new Date('2026-05-21T14:30:00Z')
      ...
      // Дефолтно маркетинговые email выключены
      expect(result.decision).toBe(Decision.DENY);
      expect(result.reason).toBe(DenyReason.USER_DISABLED);
    });
  });
